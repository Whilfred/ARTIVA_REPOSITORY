...
function ProductManagementPage() {
  ...
  const fetchProducts = useCallback(async () => {
    if (!adminToken) {
      console.error(
        "Admin non authentifié, impossible de charger les produits."
      );
    ...
    try {
      ...
      console.log(
        "Produits chargés pour l'admin:",
        prodResponse.data.products?.length
      );
      console.log(
        "Catégories chargées pour l'admin:",
        catResponse.data?.length
      );
    } catch (err) {
      console.error("Erreur chargement produits (admin):", err);
      setError(
        err.response?.data?.message ||
          "Impossible de charger les produits et/ou catégories."
      );
      setAllProducts([]);
      setAllCategories([]);
    } finally {
  ...
  const filteredProducts = useMemo(() => {
    ...
    return allProducts.filter(
      (product) =>
        ...
        (product.id &&
          String(product.id).toLowerCase().includes(lowerSearchTerm))
    );
  ...
  const handleTogglePublishStatus = async (productId, currentStatus) => {
    ...
    if (window.confirm(`Voulez-vous vraiment ${action} ce produit ?`)) {
      ...
      try {
        ...
        console.error(
          `Erreur lors de la tentative de ${action} le produit:`,
          err
        );
        setError(
          err.response?.data?.message ||
  ...
  return (
    <div className="product-management-page">
      ...
      {isLoading && searchTerm && (
        <p className="loading-indicator">Recherche en cours...</p>
      )}
              ...
              <tr>
                ...
                >
                  {searchTerm
                    ...
                    : "Aucun produit à afficher. Cliquez sur '+ Ajouter Produit' pour commencer."}
                </td>
              </tr>
            )}
          </tbody>
