...
const API_BASE_URL =
  process.env.REACT_APP_API_URL || "https://artiva-repository.onrender.com/api"; // Adaptez à votre IP/Port backend
...
const ScanOrderPage = () => {
  ...
  const handleScanSuccess = useCallback(
    async (decodedText) => {
      ...
      if (isNaN(orderId)) {
        setError(
          `Format QR Code invalide: "${decodedText}". Le QR Code doit contenir l'ID numérique de la commande.`
        );
        setIsLoading(false);
      ...
      try {
        ...
        if (!token) {
          ...
          setError(
            "Token administrateur non trouvé. Veuillez vous reconnecter."
          );
          setIsLoading(false);
          navigate("/login");
          return;
        ...
        if (!response.ok) {
          ...
          if (response.status === 404) {
            throw new Error(
              `Commande avec ID ${orderId} non trouvée dans la base de données.`
            );
          }
          ...
          throw new Error(
            errorData.message ||
              `Erreur ${response.status}: Impossible de récupérer les détails de la commande.`
          );
        }
    ...
    [navigate]
  ); // navigate est une dépendance pour useCallback
  ...
  const handleUpdateOrderStatus = async (orderId, newStatus) => {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(
          responseData.message ||
            `Erreur ${response.status} lors de la mise à jour du statut.`
        );
      }
      ...
      setUpdateStatusSuccess(
        `Statut de la commande mis à jour à "${responseData.order.status}".`
      );
    } catch (err) {
  ...
  const styles = {
    ...
    cardTitleContainer: {
      // Conteneur pour titre et bouton d'action
      display: "flex",
  ...
  return (
    <div style={styles.pageContainer}>
      <h1 style={styles.header}>
        <ScanLine
          size={30}
          style={{ marginRight: "12px", color: styles.header.color }}
        />
      ...
      {scanActive &&
        !orderDetails &&
        !error && ( // Ajouter !error pour ne pas montrer le scanner si une erreur de scan (parsing ID) est là
          <div style={styles.scannerSection}>
            <p style={styles.infoText}>
              Placez le QR Code de la commande devant la caméra.
            </p>
        ...
        <div style={styles.resultsSection}>
          ...
          <div style={styles.card}>
            <div style={styles.cardTitleContainer}>
              {" "}
              {/* Utiliser le nouveau conteneur pour le titre et le bouton */}
              <h3
                style={{
                  ...styles.cardTitle,
                  marginBottom: 0,
              ...
              {canMarkAsDelivered && (
                <button
                  onClick={() =>
                    handleUpdateOrderStatus(orderDetails.orderId, "delivered")
                  }
            ...
            <p style={styles.detailItem}>
              <span style={styles.detailLabel}>Numéro Commande:</span>{" "}
              <span style={styles.detailValue}>
                {orderDetails.order_number}
              </span>
            </p>
            <p style={styles.detailItem}>
              <span style={styles.detailLabel}>ID Interne:</span>{" "}
              <span style={styles.detailValue}>{orderDetails.orderId}</span>
            </p>
            <p style={styles.detailItem}>
              <span style={styles.detailLabel}>Statut:</span>{" "}
              <span style={styles.statusBadge(orderDetails.status)}>
                {orderDetails.status}
              </span>
            </p>
            <p style={styles.detailItem}>
            ...
            {orderDetails.notes && (
              <p style={styles.detailItem}>
                <span style={styles.detailLabel}>Notes Client:</span>{" "}
                <span style={styles.detailValue}>{orderDetails.notes}</span>
              </p>
            )}
          </div>
          {/* Card Client */}
          <div style={styles.card}>
            <h3 style={styles.cardTitle}>
              <UserCircle size={20} style={{ marginRight: "10px" }} />
              Client
            </h3>
            <p style={styles.detailItem}>
            ...
            <p style={styles.detailItem}>
              ...
              <span style={styles.detailValue}>
                {orderDetails.userEmail || "N/A"}
              </span>
            </p>
            <p style={styles.detailItem}>
              <span style={styles.detailLabel}>Téléphone:</span>{" "}
              <span style={styles.detailValue}>
                {orderDetails.userPhone || "N/A"}
              </span>
            </p>
          </div>
            ...
            <div style={styles.card}>
              ...
              {(() => {
                ...
                return (
                  <>
                    <p style={styles.detailItem}>
                      <span style={styles.detailLabel}>Destinataire:</span>{" "}
                      <span style={styles.detailValue}>{addr.name}</span>
                    </p>
                    <p style={styles.detailItem}>
                    ...
                    <p style={styles.detailItem}>
                      ...
                      <span style={styles.detailValue}>{addr.city}</span>
                    </p>
                    <p style={styles.detailItem}>
                      <span style={styles.detailLabel}>Code Postal:</span>{" "}
                      <span style={styles.detailValue}>
                        {addr.postal_code || "N/A"}
                      </span>
                    </p>
                    <p style={styles.detailItem}>
                      <span style={styles.detailLabel}>Pays:</span>{" "}
                      <span style={styles.detailValue}>{addr.country}</span>
                    </p>
                    <p style={styles.detailItem}>
                      <span style={styles.detailLabel}>Tél. Livraison:</span>{" "}
