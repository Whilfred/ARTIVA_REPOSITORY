  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (tagToEdit && tagToEdit.id) {
        // Mode Édition
        await axios.put(
          `${apiBaseUrl}/product-tags/${tagToEdit.id}`,
          { name: tagName },
          {
            headers: {
      ...
      setError(
        err.response?.data?.message || "Erreur lors de la sauvegarde du tag."
      );
    } finally {
  ...
  return (
    <div className="modal-overlay" onClick={onClose}>
      ...
      <div className="modal-content" onClick={(e) => e.stopPropagation()}>
        {" "}
        {/* Empêche la fermeture si on clique dans le modal */}
...
function ProductTagsPage() {
  ...
  const API_BASE_URL = "https://artiva-repository.onrender.com/api"; // Assure-toi que c'est correct
  ...
  const fetchTags = useCallback(async () => {
    ...
    try {
      ...
      setError(
        err.response?.data?.message || "Impossible de charger les tags."
      );
      setTags([]);
    } finally {
      setIsLoading(false);
  ...
  const handleDeleteTag = async (tagId, tagName) => {
    if (
      window.confirm(
        `Êtes-vous sûr de vouloir supprimer le tag "${tagName}" ? Les produits ne seront plus associés à ce tag.`
      )
    ) {
      ...
      try {
        ...
        setError(
          err.response?.data?.message || "Erreur lors de la suppression du tag."
        );
      } finally {
        setIsLoading(false);
      }
  ...
  return (
    <div className="management-page">
      <div className="page-header">
        ...
        <button onClick={handleOpenModalForAdd} className="add-btn">
          + Ajouter Tag
        </button>
      </div>
              ...
              tags.map((tag) => (
                <tr key={tag.id}>
                  ...
                  <td className="actions-cell">
                    <button
                      onClick={() => handleOpenModalForEdit(tag)}
