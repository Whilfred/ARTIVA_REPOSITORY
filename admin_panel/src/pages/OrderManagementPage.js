...
const API_BASE_URL = "https://artiva-repository.onrender.com/api"; // Déplacé en haut pour la portée globale du module
...
function OrderManagementPage() {
  ...
  const fetchOrders = useCallback(
    async (
      ...
      console.log(
        `fetchOrders appelé avec page: ${pageToFetch}, status: ${currentFilterStatus}, user: ${currentFilterUserId}, from: ${currentDateFrom}, to: ${currentDateTo}`
      );
      if (!adminToken) {
        navigate("/login");
        return;
  ...
  ); // ITEMS_PER_PAGE est une constante, donc pas besoin de la mettre si elle est hors du composant

  // useEffect principal pour charger les données quand les filtres ou la page changent
  useEffect(() => {
    console.log(
      "useEffect principal [currentPage, filterStatus, ...] déclenché - Appel de fetchOrders"
    );
    fetchOrders(
      currentPage,
      filterStatus,
  ...
  const handleStatusChange = async (orderId, newStatus) => {
    ...
    if (
      newStatus === "cancelled" &&
      !window.confirm("Confirmer l'annulation de cette commande ?")
    )
      return;
  ...
  if (isLoading && !initialLoadAttempted) {
    // Afficher seulement au tout premier chargement
    return (
      <div
  ...
  return (
    <div className="management-page">
      ...
      {isLoading && initialLoadAttempted && (
        <p className="loading-indicator">Mise à jour des données...</p>
      )}
      ...
      {totalPages > 1 && (
        ...
        >
          ...
          <span>
            Page {currentPage} sur {totalPages} ({totalItems} commandes)
          </span>
          <button
